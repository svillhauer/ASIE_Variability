function [v,p,V,VAR,CI,beta1,beta2,xx_new] = csapsGCV_with_Phase(x,y,p,xx,W,beta1,beta2)
% Phase adjustment added by Sarah Villhauer 
...
N = length(x);
if nargin < 5, W = []; end
% Set default values if beta1 and beta2 are not provided
if nargin < 6, beta1 = 1; end
if nargin < 7, beta2 = 1; end
...
if (nargin < 4)||(isempty(xx))
    ...
    xx_new = 365 * betacdf(xx, beta1, beta2);
    %xx_new = [linspace(0,xx_new(1),length(1:index_first)) xx_new(2:end)];
    %xx_new = [xx_new(1:end-1) linspace(xx_new(end),364, length(index_last:365))];
    tt = (xx_new-min(x))/(max(x)-min(x));  % scales the shifted values so that the range becomes [0, 1]
end
...
% Define the initial guess for beta1 and beta2
initial_guess = [P(iiV), beta1, beta2];

% Estimate optimal values of smoothing parameter, beta1, and beta2
if (nargin < 3)||(isempty(p))
    options = optimset('TolFun',(1e-3)*V(iiV),'TolX',(1e-3)*P(iiV));
    if N > 30
        params = fminsearch(@(params) GCV(y,h,params(1),N,W,params(2),params(3)),initial_guess,options);
    else
        params = fminsearch(@(params) OCV(y,t,params(1),N,diag(W),params(2),params(3)),initial_guess,options);
    end
    p = params(1);
    beta1 = params(2);
    beta2 = params(3);
...
[V,VAR,CI] = GCV(y,h,p,N,W,beta1,beta2);
...
function [V,VAR,CI] = GCV(y,h,p,N,W,beta1,beta2)
% Generalized cross-validation with beta1 and beta2
Q = return_differences(h,N);
T = return_T(h);
B = calc_B(Q,W,p,T);
...
function [V,VAR,CI] = OCV(y,t,p,N,W,beta1,beta2)
